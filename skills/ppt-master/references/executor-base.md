...
### 7.1 Font Size Hard Floor (Mandatory)

**Hard rule**: every `<text>` / `<tspan>` element in every SVG page MUST meet the floor below. The floor is absolute — a single undersized element triggers an `svg_quality_checker.py` error and blocks downstream export.

| Role | Floor (px) | Notes |
|---|---|---|
| **H1** — main title (`cover_title` / page title) | **≥ 40px** | Holds the strongest visual weight on the page. Below 40 reads as body. |
| **H2** — subtitle (`subtitle` / section opener) | **≥ 32px** | Sits between H1 and body. Below 32 collapses into body weight. |
| **Body** — running prose | **≥ 24px** | Applies to bullets, paragraphs, KPI labels. Below 24 is annotation territory. |
| Annotation / footnote | ≥ 12px | Acceptable ONLY for source citations, page numbers, formula index — not body content. |

**Dark-theme buffer** (recommended when `## Theme kind = dark` in `spec_lock.md`): bump each floor by +4px to compensate for projector / screen luminance loss:

| Role | Dark-theme recommendation |
|---|---|
| H1 | 44px |
| H2 | 34px |
| Body | 26px |

**Forbidden — undersized text**:

| Anti-pattern | Consequence |
|---|---|
| `<text font-size="18">核心結論</text>` | Below body floor; `svg_quality_checker.py` errors |
| `<text font-size="14">長段說明文字</text>` | Annotation territory applied to running prose |
| `<text font-size="32">主標題</text>` | H1 should be ≥ 40, not 32 (32 is H2 territory) |

**Allowed — floor met**:

| Pattern | Why it passes |
|---|---|
| `<text font-size="44">主標題</text>` | H1 ≥ 40 (dark-theme recommended) |
| `<text font-size="34">副標題</text>` | H2 ≥ 32 (dark-theme recommended) |
| `<text font-size="26">說明文字</text>` | Body ≥ 24 (dark-theme recommended) |

**Decision tree when the floor feels too large for the layout**: reduce the *content* (fewer lines / smaller words), not the font size. Under-floor typography is a content-cut problem, not a font-size problem.

**Note on `page_rhythm: breathing`**: the floor still applies — a breathing page with a single large quote still needs its quote at H1 floor (≥ 40px). Breathing affects layout density, not font scale.

---

## 8. Color Token Discipline (No Hex Literals)

**Hard rule**: SVG elements (`<rect>`, `<circle>`, `<path>`, `<line>`, `<text>`, `<stop>`, `<tspan>`, `<polygon>`, etc.) MUST NOT contain literal HEX color values like `#RRGGBB` / `#RGB` / `rgb(...)` / `rgba(...)` / `hsl(...)`. Use CSS custom property references — `var(--token-name)` — and resolve them at finalize time from the theme declared in `spec_lock.md`.

**Why this rule exists**: theme is the single source of truth. If a page hard-codes `#5AC8FA` mid-document, a later theme change requires a multi-file grep. Token references mean one spec_lock edit re-themes the whole deck.

### 8.1 Token source — `spec_lock.md ## Theme`

Each deck has exactly one theme. Read it from `<project>/spec_lock.md ## Theme`:
...
Executor resolves the table into a token map at finalize time:
...
Then writes a `<style>` block at the top of the final SVG:
...
Author-time SVG elements reference tokens:

```xml
<rect x="0" y="0" width="1280" height="720" fill="var(--bg-canvas)"/>
<text x="60" y="100" font-size="44" fill="var(--text-primary)">主標題</text>
<circle cx="640" cy="360" r="120" fill="var(--accent)"/>
```

### 8.2 Whitelist (literal values allowed)

| Value | Where allowed | Why |
|---|---|---|
| `transparent` | `fill` / `stroke` | CSS keyword, not a color value |
| `currentColor` | `fill` / `stroke` | Inherits `color` from cascade — useful for icons that should match the parent text color |
| `none` | `fill` / `stroke` | CSS keyword — "no paint applied" |
| `inherit` | `fill` / `stroke` | Inherits from parent element |
| `<icon-name>` library color value (`fill="#5AC8FA"` on `<use data-icon="...">`) | icon placeholders only | Icon library contract — see §4 (Icon Usage) |

### 8.3 Forbidden — hex literals in authored SVG

| Anti-pattern | Why it fails |
|---|---|
| `<rect fill="#5AC8FA" />` | Bypasses the theme system; later re-themes miss this file |
| `<text fill="#F5F5F5">主標題</text>` | Same — couples the text color to a single theme |
| `<stop stop-color="#FF9F0A" />` (inside `<linearGradient>`) | Same — gradients must also reference tokens |
| `<rect fill="rgb(245,245,245)" />` | Same — `rgb()` is a hex literal in disguise |
| `<circle fill="#fff" />` | Same — even shorthand hex counts as a literal |

**Allowed equivalent**:

| Pattern | Why it passes |
|---|---|
| `<rect fill="var(--accent)" />` | Token reference, theme-aware |
| `<text fill="var(--text-primary)">主標題</text>` | Token reference, theme-aware |
| `<stop stop-color="var(--accent-warm)" />` | Gradients must also tokenize |
| `<rect fill="transparent" />` | Whitelisted keyword |
| `<rect fill="none" />` | Whitelisted keyword |
| `<use data-icon="chunk-filled/home" fill="var(--accent)" />` | Icon placeholder with token reference |

### 8.4 Edge cases — when a token reference is not feasible

| Case | Handling |
|---|---|
| `currentColor` cascade — `<use>` inside `<g fill="var(--accent)">` | The `<g>` sets `fill="var(--accent)"`, child `<use>` uses `fill="currentColor"` to inherit. Both are tokenized. |
| External assets (PNG / JPG) carrying baked-in color | Outside the rule — bitmaps don't have token resolution. Document in `spec_lock.md` if many pages depend on a baked color. |
| Stroke / shadow with alpha (e.g., `var(--accent)` at 20% opacity) | Use `fill-opacity="0.2"` on the element, do NOT bake the alpha into a custom token. Tokens stay solid HEX. |
| Mid-page brand override (one card uses a brand-specific hex) | Declare a per-card override token in `spec_lock.md` (`accent-brand: #ABC123`); reference it as `var(--accent-brand)`. Never inline the brand hex. |

### 8.5 Validation hook

`svg_quality_checker.py` scans every authored SVG in `svg_output/` for `#RRGGBB` / `#RGB` / `rgb(...)` / `rgba(...)` / `hsl(...)` literals inside `fill` / `stroke` / `stop-color` attributes and reports them as **errors**. Fix the offending element to use a `var(--token)` reference. The icon `fill="#HEX"` exception is gated to `<use data-icon="...">` elements only.

**Forbidden — silencing the checker**: do not add the offending file to a checker exclusion list. Update the SVG to use a token reference, or if a new token is needed, add it to `spec_lock.md ## Theme Tokens 參考` first.
