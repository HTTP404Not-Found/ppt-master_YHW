...
try:
    ...
    # project_utils is a hard dependency: without CANVAS_FORMATS the canvas
    # viewBox checks below silently no-op on every format. Fail loudly so a
    # broken environment gets surfaced immediately rather than masquerading
    # as a passing quality check. (This replaces the old `CANVAS_FORMATS = {}`
    # fallback that drifted from the canonical definition — see v2.7.0 code
    # quality report, finding "CANVAS_FORMATS duplication drift risk".)
    raise SystemExit(
        "FATAL: cannot import project_utils.CANVAS_FORMATS — "
        "svg_quality_checker cannot validate canvas viewBoxes without it. "
        "Run from the repo root or set PYTHONPATH to include "
...
try:
    ...
    # error_helper only enriches error messages; the checker still works
    # without it, so a missing import is degraded gracefully rather than fatal.
    ErrorHelper = None
...
try:
    # Optional WCAG contrast + font-size audit. When the module is present
    # we fold its findings into the per-file report and the run summary.
    # Missing import is degraded gracefully — the rest of the checker still
    # runs without it (see v2.7.0 code quality report: graceful-degradation
...
class SVGQualityChecker:
    ...
    def check_file(self, svg_file: str, expected_format: str = None) -> Dict:
        ...
        try:
            ...
            if self._check_xml_well_formed(content, result):
                ...
                # 10. WCAG contrast + font-size audit (contrast_checker).
                #     Optional integration; runs only when the module is
                #     importable. Skipped in template mode because template
                #     SVGs use placeholder content that fails the ramp.
                if not self.template_mode:
                    self._check_contrast_and_fonts(svg_path, result)
    ...
    def _check_contrast_and_fonts(self, svg_path: Path, result: Dict) -> None:
        """Run the optional WCAG contrast + font-size audit.

        Errors come from contrast ratio < 4.5 and H1/H2/Body below the
        ramp floor; warnings come from ratios in the AA-but-not-AAA band
        and from any hex literals that fall outside an active Theme's
        token table. The audit is no-op when ``contrast_checker`` is not
        importable — the rest of the check pipeline keeps working.
        """
        if _contrast_checker is None:
            return

        contrast_violations = _contrast_checker.check_svg(svg_path)
        for v in contrast_violations:
            self._record_contrast_violation(result, v)

        font_violations = _contrast_checker.check_font_sizes(svg_path)
        for v in font_violations:
            self._record_contrast_violation(result, v)

        # No-hex-literals: advisory (warnings only — not errors). Legacy
        # decks may legitimately use hex literals while a Theme is being
        # introduced. Flag as warning so it surfaces without breaking CI.
        no_hex_violations = _contrast_checker.check_no_hex_literals(svg_path)
        for v in no_hex_violations:
            self._record_contrast_violation(result, v)

    def _record_contrast_violation(self, result: Dict, v: Dict) -> None:
        """Fold one contrast_checker violation into the per-file report.

        The mapping is intentionally direct — the source dict already has
        ``severity`` (``error``/``warning``/``info``) and a human-readable
        ``message``. We only deduplicate identical entries (same element,
        attr, color, ratio) because a single colour repeated on 30 text
        nodes otherwise floods the report.
        """
        severity = v.get("severity", "info")
        message = v.get("message") or self._format_contrast_default(v)
        if severity == "error":
            bucket = result["errors"]
        elif severity == "warning":
            bucket = result["warnings"]
        else:
            # info-level: store in result.info for the JSON/audit trail
            # without polluting the warnings bucket.
            result.setdefault("info", {}).setdefault("contrast_info", []).append(
                message
            )
            return
        # Dedupe — contrast_checker already dedupes within a single
        # function call, but cross-call collisions (e.g. same color in
        # check_svg + check_no_hex_literals) still need handling.
        if message in bucket:
            return
        bucket.append(message)

    @staticmethod
    def _format_contrast_default(v: Dict) -> str:
        parts = []
        for key in ("element", "attr", "color", "bg", "ratio", "role", "font_size"):
            if v.get(key) is not None:
                parts.append(f"{key}={v[key]}")
        return "Contrast check: " + (" | ".join(parts) if parts else str(v))
    ...
    def _categorize_issue(self, error_msg: str) -> str:
        ...
        elif error_msg.startswith('Contrast check:'):
            # Bucket all contrast / font / token findings under one label
            # so the summary stays scannable — see print_summary().
            return 'Contrast / accessibility'
