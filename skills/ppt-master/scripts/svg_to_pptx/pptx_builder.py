...
# Optional integration: token expansion + contrast audit. Missing import
# is degraded gracefully — token expansion simply isn't applied and no
# contrast_audit.json is written. The wrapper CLI in svg_to_pptx.py does
# NOT promote this to an error; legacy decks without a Theme keep working
...
def create_pptx_with_native_svg(
    ...
    try:
        ...
        # Optional: load a Theme from the project's spec_lock.md so we can
        # expand ``var(--token)`` placeholders into hex before conversion.
        # finalize_svg.py already runs the same expansion; this is a
        # double-safety so direct ``svg_to_pptx.py`` invocations (without
        # finalize_svg as a preprocessing step) also get token resolution.
        _theme: dict = {}
        _project_root: Path | None = None
        if _contrast_checker is not None and svg_files:
            # Project root = svg_files[0].parent.parent (assumes standard
            # <project>/{svg_output,svg_final}/<name>.svg layout). Falls
            # back to svg_files[0].parent if the layout doesn't match.
            first = svg_files[0]
            candidate_roots = [first.parent, first.parent.parent]
            for root in candidate_roots:
                spec = root / "spec_lock.md"
                if spec.exists():
                    _project_root = root
                    break
            if _project_root is not None:
                # Reuse the same loader pattern as finalize_svg. Inline
                # here to avoid a cross-package dependency on the wrapper.
                spec_path = _project_root / "spec_lock.md"
                try:
                ...
                for raw in text.splitlines():
                    ...
                    if not in_theme:
                        continue
                    m_theme = re.match(r"^-\s+tokens\s*:\s*(.+?)\s*$", line)
                    if m_theme:
                        value = m_theme.group(1).strip().strip('"').strip("'")
                        if value.startswith("{"):
                            try:
                                _theme["tokens"] = json.loads(value)
                            except json.JSONDecodeError:
                                pass
                        else:
                            theme_path = (spec_path.parent / value).resolve()
                            if theme_path.exists():
                                try:
                                    ...
                                    _theme = {"tokens": {}}
                    m_bg = re.match(r"^-\s+bg\s*:\s*(#[0-9A-Fa-f]{3,8})\s*$", line)
                    if m_bg:
                        _theme["bg"] = m_bg.group(1).upper()

        # Optional: per-slide contrast audit accumulator. Only populated
        # when contrast_checker is importable; written to
        # ``<output>.contrast_audit.json`` after a successful run.
        contrast_audit_slides: list[dict[str, Any]] = []

        for i, svg_path in enumerate(svg_files, 1):
            slide_num = i

            # Token expansion (double-safety — finalize_svg.py does this
            # already when invoked as a preprocess step). Skip when no
            # theme tokens were loaded.
            if _theme.get("tokens") and _contrast_checker is not None:
                try:
                    svg_text = svg_path.read_text(encoding="utf-8")
                    if "var(--" in svg_text:
                        new_text, _warnings = _contrast_checker.expand_tokens(
                            svg_text, _theme,
                        )
            ...
            # Per-slide contrast audit — accumulate all text/bg pairs with
            # their ratio + element coordinates for the final JSON report.
            if _contrast_checker is not None:
                try:
                    audit_violations = _contrast_checker.check_svg(svg_path)
                    font_violations = _contrast_checker.check_font_sizes(svg_path)
                    contrast_audit_slides.append({
                        "slide": svg_path.stem,
                        "file": svg_path.name,
                        "pairs": [
                            {
                    ...
                    # Audit is best-effort — never block export on a bug.
                    pass
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/slideLayout" Target="../slideLayouts/slideLayout1.xml"/>{extra_rels}
        ...
        # Optional contrast audit JSON — written alongside the conversion
        # trace when contrast_checker is importable. Only useful when the
        # project has a Theme or a spec_lock.md bg; otherwise the audit is
        # empty and the JSON is still emitted (so callers can detect that
        # the audit ran and decide what to do with the empty result).
        if _contrast_checker is not None:
            audit_path = output_path.with_name(
                output_path.name + '.contrast_audit.json'
            )
            try:
                audit_payload = {
                    'output': str(output_path),
                    'slide_count': len(svg_files),
                    'theme': _theme,
                    'slides': contrast_audit_slides,
                    'summary': {
                        'errors': sum(
                            len(s.get('font_size_violations', [])) +
                            sum(
                                1 for p in s.get('pairs', [])
                                if p.get('severity') == 'error'
                            )
                            for s in contrast_audit_slides
                        ),
                        'warnings': sum(
                            sum(
                                1 for p in s.get('pairs', [])
                ...
                if verbose:
                    print(f"  Contrast audit: {audit_path}")
            except Exception as exc:
                if verbose:
                    print(f"  [warn] contrast audit write skipped: {exc}")
