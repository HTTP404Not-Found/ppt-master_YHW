...
def convert_svg_to_slide_shapes(
    ...
    #
    # Placeholders that survive expansion (icon missing from the library,
    # or malformed attributes) are stripped afterwards. Without stripping,
    # the leftover <use> triggers SvgNativeConversionError below, which
    # would crash the entire export on a single bad icon. finalize_svg
    # already warns "Icon not found" when it hits this case on disk; here
    # we go one step further and remove the placeholder so the rest of
    # the slide still exports cleanly. Logged as both a warning and a
    # trace step so the operator can see what happened.
    ...
    if icons_dir.exists():
        ...
        unresolved_icons = strip_unresolved_use_data_icons(root)
        if unresolved_icons:
            trace_steps.append({
                'action': 'strip-unresolved-use-data-icons',
                'icons': list(unresolved_icons),
            })
            if verbose:
                for icon_name in unresolved_icons:
                    print(
                        f'  [warn] Icon placeholder stripped (not found in library): '
                        f'{icon_name} (in {svg_path.name})',
                        file=sys.stderr,
                    )
