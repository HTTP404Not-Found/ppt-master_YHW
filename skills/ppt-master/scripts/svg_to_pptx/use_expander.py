...
def expand_use_data_icons(root: ET.Element, icons_dir: Path) -> int:
    ...
    (or strip) them via :func:`strip_unresolved_use_data_icons`.
    """
...
def strip_unresolved_use_data_icons(root: ET.Element) -> list[str]:
    """Remove every ``<use data-icon="...">`` placeholder that survived expansion.

    Should be called after :func:`expand_use_data_icons`. Any leftover
    ``<use data-icon>`` element refers to an icon that does not exist in
    the icon library, or has malformed attributes that prevented
    resolution. PowerPoint's SVG parser and our own DrawingML dispatcher
    do not understand ``data-icon``; leaving the element in place either
    produces a useless ``<use>`` shape or crashes the native converter
    with "unsupported visual SVG element". Stripping matches the
    behaviour users expect from ``finalize_svg`` (which warns "Icon not
    found" and leaves the placeholder in svg_final/) — for the native
    PPTX export, we go one step further and remove the placeholder so
    the rest of the slide still exports cleanly.

    Returns a list of unresolved icon identifiers (``"lib/name"``) so
    callers can log them.
    """
    # Build parent map once; iter() over the tree before mutation avoids
    # the classic "list changed during iteration" ElementTree issue.
    parent_of: dict[ET.Element, ET.Element] = {}
    for parent in root.iter():
        for child in parent:
            parent_of[child] = parent
    ...
    unresolved: list[str] = []
    # Snapshot children first so we don't mutate the tree while iterating.
    for use_elem in [e for e in root.iter()
                     if (_local_tag(e) == 'use' and e.get('data-icon'))]:
        parent = parent_of.get(use_elem)
        if parent is None:
            continue
        parent.remove(use_elem)
        unresolved.append(use_elem.get('data-icon', ''))

    return unresolved


def _local_tag(elem: ET.Element) -> str:
    """Return the local (no namespace) tag name for *elem*."""
